...
ROOT_PACKAGE=github.com/openim/website
...
## init: Initialize the project (remove .git and initialize a new one).
.PHONY: init
init:
	shx rm -rf .git && git init -b main

## create: Create new Hugo content.
.PHONY: create
create:
	exec-bin node_modules/.bin/hugo/hugo new

## prestart: Clean before starting.
.PHONY: prestart
prestart:
	make clean

## start: Start the Hugo server.
.PHONY: start
start:
	exec-bin node_modules/.bin/hugo/hugo server --bind=0.0.0.0 --disableFastRender

## prebuild: Clean before building.
.PHONY: prebuild
prebuild:
	make clean
...
## build-preview: Build the Hugo site with preview options.
.PHONY: build-preview
build-preview:
	make build -D -F
...
## clean-install: Clean node modules and package-lock.
.PHONY: clean-install
clean-install:
	shx rm -rf package-lock.json node_modules

## lint: Lint scripts, styles, and markdown.
.PHONY: lint
lint:
	npm run -s lint:scripts && npm run -s lint:styles && npm run -s lint:markdown

## lint-scripts: Lint JavaScript scripts.
.PHONY: lint-scripts
lint-scripts:
	eslint assets/js config functions
...
## lint-markdown-fix: Fix markdown lint issues.
.PHONY: lint-markdown-fix
lint-markdown-fix:
	markdownlint-cli2-fix "*.md" "content/**/*.md"
...
## precheck: Check NPM version.
.PHONY: precheck
precheck:
	npm version

## check: Check Hugo version.
.PHONY: check
check:
	exec-bin node_modules/.bin/hugo/hugo version

## copy-katex-fonts: Copy KaTeX fonts to static directory.
.PHONY: copy-katex-fonts
copy-katex-fonts:
	shx cp ./node_modules/katex/dist/fonts/* ./static/fonts/

## postinstall: Run Hugo installer.
.PHONY: postinstall
postinstall:
	hugo-installer --version otherDependencies.hugo --extended --destination node_modules/.bin/hugo

## version: Generate changelog and add it.
.PHONY: version
version:
